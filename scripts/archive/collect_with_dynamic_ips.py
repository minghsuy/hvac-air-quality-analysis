...
# Configuration
SPREADSHEET_ID = os.environ.get('GOOGLE_SPREADSHEET_ID', '1RMhS2pra8Fho3mw0RMc39IMT-seBeTN_tmo9yFIwBBo')
CREDENTIALS_FILE = '/data/scripts/google-credentials.json' if os.path.exists('/data/scripts/google-credentials.json') else 'google-credentials.json'
...
def ensure_headers(service, spreadsheet_id):
    try:
        ...
        headers = [
            'Timestamp', 'Sensor_ID', 'Room', 'Sensor_Type',
            'Indoor_PM25', 'Outdoor_PM25', 'Filter_Efficiency',
            'Indoor_CO2', 'Indoor_VOC', 'Indoor_NOX',
...
def main():
    ...
    if master:
        ...
        row = [
            timestamp, master['sensor_id'], master['room'], master['sensor_type'],
            master['pm25'], outdoor_pm25, efficiency,
            master['co2'], master['voc'], master['nox'],
            master['temp'], master['humidity'], master['radon'],
            outdoor['co2'], outdoor['temp'], outdoor['humidity'],
            outdoor['voc'], outdoor['nox']
        ]
    ...
    if AIRGRADIENT_INDOOR_SERIAL:
        ...
        if second:
            ...
            row = [
                timestamp, second['sensor_id'], second['room'], second['sensor_type'],
                second['pm25'], outdoor_pm25, efficiency,
                second['co2'], second['voc'], second['nox'],
                second['temp'], second['humidity'], second['radon'],
                outdoor['co2'], outdoor['temp'], outdoor['humidity'],
                outdoor['voc'], outdoor['nox']
            ]
            rows_to_append.append(row)
