...
def analyze_filter_change_impact(df):
    ...
    day_before = df[(df['timestamp'] >= filter_change - timedelta(hours=24)) & 
                    (df['timestamp'] < filter_change)]
    
    # Get the first 24 hours after change  
    day_after = df[(df['timestamp'] >= filter_change) & 
                   (df['timestamp'] < filter_change + timedelta(hours=24))]
    ...
    if len(day_after) > 0:
        ...
        if len(day_before) > 0 and len(day_after) > 0:
            t_stat, p_value = stats.ttest_ind(
                day_before['Filter Efficiency'].dropna(), 
                day_after['Filter Efficiency'].dropna()
            )
            print(f"\n📈 Statistical Test (t-test):")
            print(f"  t-statistic: {t_stat:.2f}")
            print(f"  p-value: {p_value:.4f}")
            if p_value < 0.05:
                improvement = day_after['Filter Efficiency'].mean() - day_before['Filter Efficiency'].mean()
                print(f"  ✅ Significant change: {improvement:+.1f}% efficiency")
            else:
                print(f"  ℹ️ No statistically significant change detected")
...
def create_comprehensive_plots(df, filter_change):
    ...
    fig.add_shape(
        type="line",
        x0=filter_change, x1=filter_change,
        y0=0, y1=100,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=['After' if t > filter_change else 'Before' 
                  for t in plot_data['timestamp']],
    ...
    # 5. Rolling average
    plot_data['eff_rolling'] = plot_data['Filter Efficiency'].rolling(
        window=72, min_periods=1, center=True
    ).mean()
    ...
    # Add WHO guideline
    fig.add_hline(
        y=15, line_dash="dot", line_color="red",
...
def main():
    ...
    # Visualize
    fig = create_comprehensive_plots(df, filter_change)
