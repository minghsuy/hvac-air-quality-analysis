...
3. From your Mac:
   ```bash
   ssh root@192.168.0.1   # or the username you set
   ```
...
5. Reconnect with the key:
   ```bash
   ssh -i ~/.ssh/id_ed25519 root@192.168.0.1
   ```
...
```bash
ssh-copy-id -i ~/.ssh/id_ed25519.pub <username>@192.168.0.1
```
...
```bash
ssh -i ~/.ssh/id_ed25519 <username>@192.168.0.1
```
...
| `Host key verification failed` | `ssh-keygen -R 192.168.0.1` to clear old fingerprint.               |
