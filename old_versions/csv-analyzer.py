...
def analyze_air_quality(df):
    ...
    for metric in available:
        ...
        if metric == 'pm25':
            ...
            print(f"  Days above WHO limit ({who_limit} μg/m³): {days_above} ({days_above/len(df)*100:.1f}%)")
        elif metric == 'co2':
            high_co2 = 1000  # Cognitive impact threshold
            hours_above = (df[metric] > high_co2).sum()
            print(f"  Hours above {high_co2} ppm: {hours_above} ({hours_above/len(df)*100:.1f}%)")
...
def plot_timeline(df, metric='pm25'):
    ...
    for event, date in events.items():
        ...
        if df.index.min() <= event_date <= df.index.max():
            ...
            plt.text(event_date, plt.ylim()[1]*0.95, event, 
...
def predict_replacement(df):
    ...
    if 'pm25' in recent_data.columns:
        ...
        if slope > 0:
            ...
            print(f"Recommended replacement date: {datetime.now().date() + pd.Timedelta(days=days_until_target)}")
        else:
