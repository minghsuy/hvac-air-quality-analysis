...
### Added
- **`Indoor_Pressure` column** (#39). Airthings View Plus exposes barometric pressure (`sensorType: "pressure"`, mbar/hPa, verified live as 1011.6 mbar at our device). `get_airthings_data()` now collects it (both new- and legacy-format API branches, empty-string sentinel for missing-vs-zero unambiguity). Sheet schema bumped to 19 columns: `EXPECTED_COLS = 19`, `SHEET_RANGE = "A:S"`, headers list and all four hardcoded `A:R` / `A1:R1` references updated. Pre-deploy 18-col rows naturally pad to NaN for `Indoor_Pressure`. Three new tests: 19-col modern row preserves pressure value, 18-col modern row gets NaN without shift-repair, and **18-col pre-stabilization row preserves values without shift-repair** (regression guard for the next bullet).
- **Decoupled shift-repair threshold from current schema width** (#39). Introduced `LEGACY_SHIFT_THRESHOLD = 18` so shift-repair only fires on true 17-col legacy rows. Previously the gate was `orig_cols < EXPECTED_COLS`; bumping `EXPECTED_COLS` to 19 would have started shift-repairing 536 pre-stabilization 18-col rows that were already complete (verified directly against the Sheet — these rows had all legacy fields populated despite being pre-`2025-09-01`). Future schema additions (col 20, 21, …) now widen `EXPECTED_COLS` and `SHEET_RANGE` without retroactively re-classifying historical rows as shift-repair candidates.
